...
IkarusEntitySetNameResult ikarus_entity_set_name_v1(Project * project, Id entity, char const * new_name, IkarusEntitySetNameV1Flags flags) {
    ...
    util::update_single_string(project, entity, "entity", "name", "id", new_name, &ret.status_code);
...
IkarusEntitySetInformationResult ikarus_entity_set_information_v1(
    ...
    util::update_single_string(project, entity, "entity", "information", "id", new_information, &ret.status_code);
...
IkarusEntityGetLocationResult ikarus_entity_get_location_v1(Project * project, Id entity, IkarusEntityGetLocationV1Flags flags) {
    ...
    VTRYRV(
        ...
        db::get_one<Id, size_t>(
            project->db, &ret.status_code, "SELECT IFNULL(`parent_folder`, 0), `position` FROM `entity_tree` WHERE `entity_id` = ?", entity
        )
    );
...
IkarusEntitySetLocationResult ikarus_entity_set_location_v1(
    ...
    VTRYRV(
        auto scope,
        ret,
        db::get_one<std::optional<Id>>(project->db, &ret.status_code, "SELECT `scope` FROM `entity_tree` WHERE `id` = ?", entity)
    );
